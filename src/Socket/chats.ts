...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	// [PATCH-021] cherry-pick Baileys 3730684e — registerSocketEndHandler agregado no destructure
	const {
		ev,
		ws,
		authState,
	...
	const updateBlockStatus = async (jid: string, action: 'block' | 'unblock') => {
		// [PATCH-018] cherry-pick Baileys 8ca9316a — valida JID e resolve par PN↔LID
		// antes de mandar o iq de blocklist. Sem isso, blocking de número malformado
		// ou de LID sem PN mapeado dava 'bad-request' opaco do servidor.
	...
	// [PATCH-021] cherry-pick Baileys 3730684e — release per-instance state no socket-end:
	//   - awaitingSyncTimeout: clear pra que reconnect não dispare warn fantasma
	//   - placeholderResendCache: só fecha se config não passou cache externo
	//   - syncState: reset para Connecting (próximo connect re-inicializa)
	//   - privacySettings: reset (próximo connect re-fetch do server)
	registerSocketEndHandler(() => {
		if (awaitingSyncTimeout) {
			clearTimeout(awaitingSyncTimeout)
			awaitingSyncTimeout = undefined
