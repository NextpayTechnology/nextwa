/**
 * [PATCH-005] Reporting token utilities — portado de whaileys.
 *
 * O WhatsApp exige um `reporting_token` anexo a certas mensagens (tudo que
 * não for reaction / poll-update). Esse token é gerado por HMAC sobre campos
 * específicos do proto encodado + uma chave derivada do `messageSecret`.
 *
 * Sem isso:
 *   - Mensagens ricas (interactiveMessage, buttonsMessage, templateMessage,
 *     listMessage) são ACEITAS pelo servidor do WhatsApp mas DESCARTADAS
 *     silenciosamente — o remetente vê "✓" local mas nada aparece no
 *     destinatário.
 *
 * O Baileys upstream v6.7.21 removeu este helper. Whaileys mantém.
 * Portado literalmente de `whaileys/lib/Utils/reporting-utils.js`.
 */
...
type FieldCfg = { f: number; m?: boolean; s?: FieldCfg[] }

const reportingFields: FieldCfg[] = [
	{ f: 1 },
	{
		f: 3,
		s: [
...
/**
 * Decide se esta mensagem deve carregar um reporting token.
 * Regra: tudo EXCETO reactions e poll-updates.
 */
...
// ─── Varint helpers (wire protocol protobuf) ────────────────────────────────

type VarintResult = { value: number; bytes: number; ok: boolean }

const decodeVarint = (buffer: Buffer | Uint8Array, offset: number): VarintResult => {
	let value = 0
	let bytes = 0
	let shift = 0
...
const compileReportingFields = (fields: FieldCfg[]): Map<number, CompiledField> => {
...
const EMPTY_MAP = new Map<number, CompiledField>()
const WIRE = {
	VARINT: 0,
	FIXED64: 1,
	BYTES: 2,
...
/**
 * Extrai campos protobuf relevantes (segundo `reportingFields`) para formar
 * o blob que entra no HMAC.
 *
 * Returns:
 *   - Buffer (pode ter length 0)  → parsing ok
 *   - null                        → parsing error (wire inválido / truncado)
 */
const extractReportingTokenContent = (
	data: Buffer | Uint8Array,
	cfg: Map<number, CompiledField>,
): Buffer | null => {
	const out: { num: number; bytes: Buffer | Uint8Array }[] = []
	let i = 0
	while (i < data.length) {
...
const generateMsgSecretKey = async (
	...
	origMsgSecret: Uint8Array | Buffer,
): Promise<Buffer> => {
	...
	return hkdf(origMsgSecret, 32, { info: useCaseSecret })
}

/**
 * Gera o `<reporting>` binary node que acompanha mensagens aceitas para
 * reporting server-side. Retorna `null` se a mensagem não qualifica
 * (sem messageSecret, sem key.id, ou proto não extratível).
 */
export const getMessageReportingToken = async (
	msgProtobuf: Buffer | Uint8Array,
